 ...
 * (`ui.visualSelector` on the body field in tina/config.ts).
 *
 * Each one is a crop of the block as it really renders on the site, regenerated by
 * scripts/generate-block-previews.mjs — so the picker shows the thing itself rather than
 * a drawing of it, and a block that changes shape is one script run from being accurate
 * again. Served from public/, which prune-dist.mjs leaves alone.
 * @type {(name: string) => string}
 */
const preview = (name) => `/block-previews/${name}.webp`
