...
// headings, the words, the links, the images, and the shape those sit in, so each
// page is reduced to those seven signals and nothing else. A diff of the snapshots
// is therefore a diff of the site as experienced, and any hit is a real regression
// rather than noise to triage.
//
...
//
// `structure` earns its place: content can be identical while the page still looks
// wrong. src/styles/layout.css addresses blocks as direct children of `.canvas`
// (`.canvas > .to-full`, `.canvas:has(> .is-flush)`), so one wrapper div around a
// block drops it out of every full-bleed rule — it renders inset, nothing fails,
// and all six content signals still match. That shipped once. It's caught here now.
...
// The element tree, as indented `tag.class.class` lines — enough to see a wrapper
// appear or a block change nesting, without the attribute churn of raw HTML.
//
// The island wrapper is unwrapped rather than reported. The editor needs a real
// element to swap re-rendered HTML into, so it is a permanent and legitimate
// difference between the two builds; it also sits outside `.canvas`, where no
// direct-child rule can reach it. Unwrapping keeps the signal at zero noise. Any
// *other* wrapper — including an anonymous <div> — shows up, which is the point.
const structure = (html) => {
  const clean = html.replace(/<(script|style)\b[^>]*>[\s\S]*?<\/\1>/gi, '').replace(/<!--[\s\S]*?-->/g, '')
  const out = []
  // One entry per open element, recording whether it was emitted — so an unwrapped
  // element still balances its close without contributing a level of indent.
  const stack = []
  const depth = () => stack.filter((e) => e.emit).length
  for (const m of clean.matchAll(/<(\/?)([a-zA-Z][\w-]*)((?:"[^"]*"|'[^']*'|[^>"'])*)(\/?)>/g)) {
    const [, closing, rawTag, attrs, selfClosed] = m
    const tag = rawTag.toLowerCase()
    if (closing) {
      // Tolerate stray closes and unclosed tags rather than desyncing the whole page.
      const i = stack.map((e) => e.tag).lastIndexOf(tag)
      if (i !== -1) stack.length = i
      continue
    }
    const emit = !/\bdata-tina-island\b/.test(attrs)
    if (emit) {
      const cls = (attrs.match(/\bclass="([^"]*)"/) ?? [])[1]
      const suffix = cls ? '.' + cls.trim().split(/\s+/).join('.') : ''
      out.push('  '.repeat(depth()) + tag + suffix)
    }
    if (!VOID.has(tag) && !selfClosed) stack.push({ tag, emit })
  }
  return out
}
